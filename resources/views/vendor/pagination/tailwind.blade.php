@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        <div class="flex justify-between flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span
                    ...
                    <i class="mr-2 fas fa-angle-double-left"></i> Anterior
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}"
                    ...
                    Urmatorul <i class="ml-2 fa fa-angle-double-right" aria-hidden="true"></i>
        ...
        <div class="flex-col hidden gap-4 sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
            ...
            <div>
                <span class="relative z-0 inline-flex rounded-md shadow-sm">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="Anterior" {{-- aria-label="{{ __('pagination.previous') }}" --}}>
                            <span
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                            ...
                            aria-label="Anterior" {{-- aria-label="{{ __('pagination.previous') }}" --}}>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                                    clip-rule="evenodd" />
                    ...
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next"
                            ...
                            aria-label="Urmatorul" {{-- aria-label="{{ __('pagination.next') }}" --}}>
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                    ...
                    @else
                        <span aria-disabled="true" aria-label="Urmatorul" {{-- aria-label="{{ __('pagination.next') }}" --}}>
                            <span
